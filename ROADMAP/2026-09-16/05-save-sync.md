...
**Gantt IDs:** `seval`, `simpl` → milestone `M3`
**Window:** 2026-12-21 → 2027-01-08
**Depends on:** `map`
...
Decide between the three options already on the table, and write the decision
down as a short ADR in `docs/`:
...
- [ ] Evaluate against: works headless under gamescope, works on Windows, no
      manual step for the kids, survives a laptop that is offline for a week.
...
- [ ] Conflict strategy: **last write wins**. The same game is not played on two
      devices in parallel here, and that is the default behaviour of both rclone
      and Syncthing.
- [ ] A stale or unreachable remote must not block play: warn, play on the local
      save, and push when the remote returns.
- [ ] Timeout on the pull step — nobody waits 60 seconds at a tile press.

## Milestone M3 — Saves survive and travel
...
- **Silent divergence.** Last-write-wins loses data if the rule is ever broken.
  Keep Ludusavi's own backup history as the undo path, and never prune it
  aggressively.
